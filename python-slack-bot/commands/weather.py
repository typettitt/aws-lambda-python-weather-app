...
class Weather:
    ...
    def process(self):
        ...
        self.location = re.findall(r'\d+|[a-z]+, [a-z]+', self.params['text'][0].lower())
    ...
    def air(self):
        ...
        air = dict["current"]["air_quality"]
        city = dict["location"]["name"]
        state = dict["location"]["region"]
        location = f"{city}, {state}"
        blocks = json.load(open(f"{self.config.MESSAGES_TEMPLATES_PATH}/air_quality.json"))
        air_quality_data = f"""```
    ...
    def alerts(self):
        ...
        state = dict["location"]["region"]
        location = f"{city}, {state}"
        if(len(dict["alerts"]["alert"]) == 0):
            text_1 = "No active alerts for " + location
            text_2 = "No alerts"
        else:
            text_1 = "Found the following alerts for " + location
            message = []
            for alert in dict["alerts"]["alert"]:
                message.append("Event: "+alert["event"]+" Effective: "+alert["effective"]+" Expires: "+alert["expires"]+"\n\n")
    ...
    def current(self):
        ...
        location = f"{city}, {state}"
        text_2 = f"""```
        Temperature: {dict["current"]["temp_f"]}F
        Feels Like: {dict["current"]["feelslike_f"]}F
        Description: {dict["current"]["condition"]["text"]}
        Wind: {dict["current"]["wind_mph"]}mph
        Humidity: {dict["current"]["humidity"]}%
        UV Index: {dict["current"]["uv"]}
        Visibility: {dict["current"]["vis_miles"]}mi
        ```"""
        blocks[0]["text"]["text"] = blocks[0]["text"]["text"] + location
        blocks[2]["text"]["text"] = text_2
        text = "Retrieving alert data!"
